...
| 버전 | 날짜 | 핵심 변화 |
|------|------|----------|
| **v2.5.2** | 2026-02-17 | 백테스팅 매 회차 자동 저장 + 로또 웹 적용 분리 |
| **v2.5.1** | 2026-02-17 | 트렌드 윈도우 축소 (100→60/20회) + 백테스팅 체크포인트 영구 저장 |
...
매 회차 자동 저장 + 로또 웹 적용 분리

문제점
- 기존: 50회 단위 저장 → 중간에 브라우저 종료 시 최대 50회 손실
- 기존: 백테스팅 결과가 자동으로 로또 웹에 적용 → 의도치 않은 덮어쓰기 위험

변경사항

저장 구조 완전 분리

| 용도 | Firebase 문서 | 저장 시점 | 설명 |
|------|--------------|----------|------|
| **백테스팅 진행도** | `backtest_progress` | 매 회차마다 자동 | 중단 후 재개용 (독립적) |
| **로또 웹 학습** | `shared_engine_state` | 수동 (버튼 클릭) | 실제 로또 웹 적용용 |

매 회차 자동 저장
- 모든 회차 실행 후 즉시 Firebase + 로컬 스토리지 저장
- 저장 내용: `currentProbMap`, `cumulativeHistory`, `cumulativeBonus`, `nextIdx`, `btStats` 등
- 브라우저 종료 시 데이터 손실 0%
- 멀티 디바이스 지원 (PC → 스마트폰 → 태블릿 자유롭게 이동)

자동 복원 (페이지 로드 시)
```
① Firebase에서 backtest_progress 로드 시도
② 로컬 스토리지에서 체크포인트 로드 시도  
③ 둘 중 최신 것 자동 선택 (savedAt 비교)
④ BT_STATE 복원
⑤ "▶ 중단된 지점부터 계속" 버튼 표시
```
...
로또 웹 적용 (수동)
- **"✅ 로또 웹에 적용"** 버튼 추가
- 사용자가 원할 때만 백테스팅 결과를 로또 웹에 반영
- `backtest_progress` → `shared_engine_state` 50:50 병합
- 자동 적용 안 됨 → 실수로 덮어쓸 위험 제거

UI 변경
- ❌ 제거: "50회마다 자동 계속" 체크박스 (불필요)
- ✅ 유지: "전체 완료 시 자동 반복" 체크박스
- ✅ 추가: "💾 매 회차마다 자동 저장" 안내 문구
- ✅ 변경: "▶ 계속 (다음 50회)" → "▶ 중단된 지점부터 계속"

사용 시나리오
```
1. 백테스팅 시작 (1,200회 예정)
2. 300회 진행 중... → 브라우저 종료
3. 다음날 재진입 → "300회차까지 완료" 자동 복원
4. "▶ 계속" 클릭 → 301회부터 이어서 진행
5. 1,200회 완료 → 결과 확인
6. 만족스러우면 "✅ 로또 웹에 적용" 클릭
7. 로또 웹에 학습 결과 반영 완료!
```

기대 효과
- ✅ 데이터 손실 완전 방지 (매 회차 저장)
- ✅ 멀티 디바이스 자유로운 이동
- ✅ 백테스팅 ↔ 로또 웹 완전 분리
- ✅ 의도치 않은 덮어쓰기 방지
- ✅ 장시간 백테스팅 안전하게 수행
...
트렌드 윈도우 축소 — 최근 변화 민감도 증가

문제점
- 기존 50회 단위 트렌드 분석으로 인해 장기 패턴에 고착
- 최근 10~20회의 빠른 변화를 충분히 반영하지 못함
- 추천 번호가 특정 범위에 쏠리는 현상 발생

변경사항

| 트렌드 종류 | 변경 전 | 변경 후 | 비고 |
|------------|---------|---------|------|
| **5종 패턴 트렌드** | 100회 (50+50) | 60회 (30+30) | 홀짝/AC/연속/끝수/합/고저 |
| **5종 지표 트렌드** | 100회 (50+50) | 20회 (10+10) | freq/gap/reHit/bonus |
| **색상 구역 트렌드** | 40회 (20+20) | 40회 (20+20) | 유지 |

5종 패턴 트렌드 (홀짝/AC/연속/끝수/합/고저)
- 윈도우: 100회 → 60회
- 비교: 최근30회 vs 이전30회
- 효과: 중기 패턴 변화에 더 민감하게 반응

5종 지표 트렌드 (freq/gap/reHit/bonus)
- 윈도우: 100회 → 20회
- 비교: 최신10회 vs 이전10회
- 델타 정규화 수정:
  ```javascript
  dFreq  = (최신10 - 이전10) / 10  // 기존 /50
  dGap   = (이전10 - 최신10) / 10  // 기존 /50
  dReHit = (최신10 - 이전10) / 5   // 기존 /10
  dBonus = (최신10 - 이전10) / 10  // 기존 /50
  ```
- 효과: 최근 단기 변화에 즉각 반응
