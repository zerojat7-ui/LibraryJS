...
💾 **백테스팅 체크포인트 영구 저장 (window.storage)**
- 브라우저를 나갔다 돌아와도 중단된 지점에서 이어서 진행
- 50회 청크 완료마다 `window.storage`에 자동 저장
- 페이지 로드 시 저장된 체크포인트 자동 감지 및 복원
- 복원 시 백테스팅 패널 자동 표시 + [▶ 계속] 버튼 즉시 활성화
- 처음부터 재시작 / 전체 완료 시 체크포인트 자동 삭제

저장 항목

| 항목 | 설명 |
|---|---|
| `currentProbMap` | 중단 시점의 확률맵 |
| `cumulativeHistory` | 누적 당첨 번호 이력 |
| `cumulativeBonus` | 누적 보너스 번호 이력 |
| `nextIdx` | 다음 실행할 testDraws 인덱스 |
| `TRAIN_UNTIL` | 초기 학습 경계 회차 |
| `totalProcessed` | 전체 처리 회차 수 |
| `repeatRound` | 전체 반복 횟수 |
| `btStats` | 적중 통계 (6개/5개/4개/3개/2개 이하) |
| `btAutoNext` | 자동 계속 체크박스 상태 |
| `btRepeat` | 자동 반복 체크박스 상태 |

> `allDraws`(전체 당첨 이력)는 용량 절약을 위해 저장하지 않고, 이어가기 시 Firebase에서 자동 재로드합니다.
...
| 버전 | 날짜 | 핵심 변화 |
|------|------|----------|
| **v2.5.1** | 2026-02-17 | 백테스팅 체크포인트 영구 저장 (window.storage) |
...
**백테스팅 체크포인트 영구 저장 (index.html)**

문제점
- `BT_STATE`가 JavaScript 변수로만 존재
- 브라우저를 나가거나 탭을 닫으면 진행 상태가 초기화됨
- 50회 체크포인트에서 멈춘 뒤 재진입 시 처음부터 다시 시작해야 했음

변경사항
- `window.storage`를 이용한 체크포인트 영구 저장 구조 추가
- 50회 청크 완료마다 Firebase 저장과 동시에 로컬에도 자동 저장
- 페이지 로드 시 `tryRestoreBtCheckpoint()` 자동 실행 → 저장된 상태 복원

신규 함수 4종

| 함수 | 역할 |
|---|---|
| `saveBtCheckpoint()` | 청크 완료마다 `window.storage`에 상태 저장 |
| `loadBtCheckpoint()` | 저장된 체크포인트 읽기 |
| `clearBtCheckpoint()` | 체크포인트 삭제 (재시작·완료 시 자동 호출) |
| `tryRestoreBtCheckpoint()` | 페이지 로드 시 자동 복원 시도 |

체크포인트 삭제 시점

| 상황 | 동작 |
|---|---|
| 처음부터 재시작 | `clearBtCheckpoint()` 후 재시작 |
| 전체 데이터 소진 완료 | `clearBtCheckpoint()` 자동 호출 |
| 자동 반복 모드 | 반복 중에는 체크포인트 유지 |
