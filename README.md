...
|       |      | **⑧ meta.version 필드 추가** |
| **2.1.1** | **2026-02-16** | **[버그픽스] ① 초기 시작 점수 0 표시 문제 — Firebase 로드 직후 이전 bestScore를 KPI·차트에 즉시 반영** |
|       |      | **[버그픽스] ② poolSize 하드코딩 문제 — 슬라이더(500~10,000) 추가, generate()에 값 전달** |
|       |      | **[버그픽스] ③ 변화 번호 수 KPI 누락 — renderProbDiff에서 변화 번호 수를 별도 KPI로 표시** |
|       |      | **[개선] Firebase 저장 pool 100개 → 500개로 확대** |
|       |      | **[개선] KPI에 이전(Firebase) 점수 항목 추가** |
